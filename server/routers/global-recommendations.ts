...
import { eq, and, desc, sql, or, ne, isNotNull,  type SQL } from "drizzle-orm";
  ...
  getGlobalRecommendations: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        if (userId) {
          whereConditions.push(ne(savedOutfits.userId, userId));
        }

        if (input.styleCategory) {
          whereConditions.push(eq(savedOutfits.style, input.styleCategory));
        }
        ...
        if (userId) {
          ...
          const likedStyleList = userLikedStyles
            .map(s => s.style)
            .filter((style): style is string => style !== null); // Filter out null styles

          const likedStyleConditions: SQL[] = likedStyleList.map(style => eq(savedOutfits.style, style));
          if (likedStyleConditions.length > 0) {
            whereConditions.push(or(...likedStyleConditions) as SQL);
          }
        }
