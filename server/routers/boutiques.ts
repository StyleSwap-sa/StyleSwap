...
export const boutiquesRouter = router({
  ...
  /**
   * Verify boutique email with token
   */
  verifyEmail: publicProcedure
    .input(z.object({ token: z.string() }))
    .mutation(async ({ input }) => {
      const { verifyBoutiqueEmail } = await import("../email.verification");
      const result = await verifyBoutiqueEmail(input.token);
  ...
  resendVerificationEmail: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!userRole || userRole.role !== "owner") {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "Only boutique owners can resend verification emails",
        });
      }

      const { resendVerificationEmail } = await import("../email.verification");
      const result = await resendVerificationEmail(input.boutiqueId);
  ...
  getVerificationStatus: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        isVerified: boutique.isVerified === 1,
