...
export function VirtualTryOnUpload() {
  ...
  const handleCreateTryOn = async () => {
    ...
    try {
      ...
      if (modelDimensions.width > 1024 || modelDimensions.height > 1024) {
        console.log("[VirtualTryOnUpload] Model photo exceeds 1024px, auto-resizing...");
      ...
      console.log("[VirtualTryOnUpload] Processing clothing image for type:", clothType);
      
      try {
        if (clothType === "upper") {
          console.log("[VirtualTryOnUpload] Cropping top portion of clothing image");
          ...
          console.log("[VirtualTryOnUpload] Cropping bottom portion of clothing image");
        ...
        console.error("[VirtualTryOnUpload] Error cropping image:", cropError);
        // Continue with original image if cropping fails
        setWarning("Could not optimize clothing image, using original");
      }
      
      setProcessingProgress(15);
      console.log("[VirtualTryOnUpload] Ready to send images to backend");
      ...
      console.log("[VirtualTryOnUpload] FormData clothType:", clothType);
      console.log("[VirtualTryOnUpload] FormData modelImage:", finalModelPhoto?.name, finalModelPhoto?.size);
      console.log("[VirtualTryOnUpload] FormData clothImage:", finalClothImage?.name, finalClothImage?.size);
  ...
  useEffect(() => {
    ...
    if (elapsedTime > POLLING_TIMEOUT_MS) {
      console.log("[VirtualTryOnUpload] Polling timeout after", elapsedTime, "ms");
    ...
    const pollStatus = async () => {
      try {
        ...
        if (data.status?.toUpperCase() === "COMPLETED") {
          console.log("[VirtualTryOnUpload] Try-on completed successfully!");
          setResult({
            taskId: data.taskId,
        ...
        console.error("[VirtualTryOnUpload] Polling error:", err);
      }
    };
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground">
            Test all clothing types (no credits used)
          </p>
        </div>
