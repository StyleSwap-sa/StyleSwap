...
export const boutiqueTransactions = pgTable("boutiqueTransactions", {
	id: serial().primaryKey().notNull(),
	boutiqueId: serial().notNull().references(() => boutiques.id),
	type: boutiqueTransactionTypeEnum.notNull(),
	...
	description: text(),
	status: boutiqueTransactionStatusEnum.default('pending').notNull(),
	createdAt: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
},
(table) => [
...
export const boutiqueUsers = pgTable("boutiqueUsers", {
	...
	role: boutiqueUserRoleEnum.default('staff').notNull(),
	createdAt: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
},
(table) => [
...
export const boutiques = pgTable("boutiques", {
	...
	status: boutiqueStatusEnum.default('active').notNull(),
...
export const emailNotifications = pgTable("emailNotifications", {
	id: serial().primaryKey().notNull(),
	userId: serial().notNull().references(() => users.id),
	type: emailNotificationTypeEnum.notNull(),
	subject: varchar({ length: 255 }).notNull(),
	recipientEmail: varchar({ length: 320 }).notNull(),
	status: emailNotificationStatusEnum.default('pending').notNull(),
...
export const paymentReconciliation = pgTable("paymentReconciliation", {
	...
	reconciliationStatus: paymentReconciliationStatusEnum.default('unmatched').notNull(),
	notes: text(),
	createdAt: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
},
...
export const productSizeVariants = pgTable("productSizeVariants", {
	...
	fitAdjustment: fitAdjustmentEnum.default('perfect').notNull(),
...
export const tryOnResults = pgTable("tryOnResults", {
	...
	fitRoomRequestId: varchar({ length: 255 }),
	flowType: flowTypeEnum.default('b2c').notNull(),
...
export const users = pgTable("users", {
	...
	userType: userTypeEnum.default('customer').notNull(),
...
export const webhookAlerts = pgTable("webhookAlerts", {
	id: serial().primaryKey().notNull(),
	alertType: webhookAlertTypeEnum.notNull(),
	severity: webhookAlertSeverityEnum.default('medium').notNull(),
...
export const webhookEvents = pgTable("webhookEvents", {
	...
	status: webhookEventStatusEnum.default('pending').notNull(),
...
export const batchUploads = pgTable("batchUploads", {
	...
	status: batchUploadStatusEnum.default('pending').notNull(),
	errorMessage: text(),
	createdAt: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
...
export const batchUploadFiles = pgTable("batchUploadFiles", {
	...
	clothingType: clothingTypeEnum.notNull(),
	status: batchUploadFileStatusEnum.default('pending').notNull(),
	errorMessage: text(),
	createdAt: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
},
(table) => [
...
export const shopOrders = pgTable("shopOrders", {
	...
	status: shopOrderStatusEnum.default('pending').notNull(),
...
export const boutiqueBankAccounts = pgTable("boutiqueBankAccounts", {
	...
	accountType: bankAccountTypeEnum.default('checking').notNull(),
	isVerified: serial().default(0).notNull(),
	createdAt: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().notNull(),
},
...
export const payoutAuditLog = pgTable("payoutAuditLog", {
	...
	actorId: serial(),
	actorType: payoutAuditActorTypeEnum.default('system').notNull(),
	details: text(),
	createdAt: timestamp({ mode: 'string' }).default(sql`CURRENT_TIMESTAMP`).notNull(),
},
(table) => [
